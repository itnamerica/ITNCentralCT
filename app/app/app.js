...
myApp.controller('MainController', ['$scope', '$transitions','$http', '$anchorScroll', '$location', '$stateParams', '$timeout', '$state', '$rootScope', function ($scope, $transitions, $http, $anchorScroll, $location, $stateParams, $timeout, $state, $rootScope)  {
  ...
  $scope.submitForm = function(formType){
    ...
    $scope.loading = true;
    $http.post('/sendmail', {
      from: '"ITNCentralCT Web User" <donotreply@itnamerica.com>',
      to: 'itnamerica2018@gmail.com',
      subject: "ITNCentralCT Contact Form Submitted",
      text: $scope.formData,
  ...
  $scope.submitFormWithPDF = function(formType){
    ...
    if (formType === 'volunteer') {
        ...
        $scope.formSubject = 'ITNCentralCT - New volunteer application received';
        ...
        $scope.showPdf = true;
        $scope.formSubject = 'ITNCentralCT - New membership application received';
        $scope.generateMultiPagePDF();
    } else if (formType === 'nonrider') {
        $scope.formSubject = 'ITNCentralCT - Non-Rider application Form submitted';
        $scope.generatePDF();
    } 
  }
